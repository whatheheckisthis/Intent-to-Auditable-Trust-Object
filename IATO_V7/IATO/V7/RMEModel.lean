...
def execTrace (σ : RmeState) (calls : List RmiCall) : RmeState :=
  calls.foldl step σ

lemma step_dataCreate_eq_data
    (σ : RmeState) (rid gid : Nat)
    (hRealm : rid ∈ σ.realms)
    (hDelegated : σ.granules gid = GranuleState.Delegated) :
    (step σ (RmiCall.DataCreate rid gid)).granules gid = GranuleState.Data rid := by
  simp [step, hRealm, hDelegated, writeGranule_eq]

lemma step_dataCreate_eq_of_not_enabled
    (σ : RmeState) (rid gid target : Nat)
    (h : ¬ (rid ∈ σ.realms ∧ σ.granules gid = GranuleState.Delegated)) :
    (step σ (RmiCall.DataCreate rid gid)).granules target = σ.granules target := by
  simp [step, h]

lemma step_realmDestroy_erases_realm (σ : RmeState) (rid : Nat) :
    rid ∉ (step σ (RmiCall.RealmDestroy rid)).realms := by
  simp [step]
...
lemma wf_execTrace (σ : RmeState) (hσ : σ.wf) (calls : List RmiCall) :
    (execTrace σ calls).wf := by
  induction calls generalizing σ with
  | nil => simpa [execTrace] using hσ
  | cons call rest ih =>
      simp [execTrace]
      exact ih (step σ call) (wf_step σ hσ call)
