...
The previous 60-file scenario corpus under `lean/formal_models/tla/scenarios/` was purged as part of repository cleanup. Use the retained baseline config `lean/formal_models/tla/dijkstra_branchless_assurance.cfg` for reproducible checks.

## Disassembly procedure

Use `scripts/formal_tla_tools/static_disassembly_check.sh` with a branchless target function.

### Example (x86_64 host)

```bash
scripts/formal_tla_tools/static_disassembly_check.sh \
  scripts/formal_tla_tools/branchless_reduction_sample.c \
  branchless_reduce \
  x86_64 \
  cc
```
...
`scripts/formal_tla_tools/branchless_reduction_arm64_asm.S` for downstream disassembly checks.
